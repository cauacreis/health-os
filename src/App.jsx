...
export default function App() {
  ...
  const loadedUserRef = useRef(null) // ← guarda o userId já carregado
  ...
  useEffect(() => {
    const { data: { subscription } } = onAuthChange(async (sess) => {
      ...
      if (sess?.user) {
        ...
        if (loadedUserRef.current === sess.user.id) return // ← já carregado, ignora
        ...
        try {
          ...
          loadedUserRef.current = sess.user.id // ← marca como carregado
        }
        ...
        setProfile(null)
        fetchingRef.current = false
        loadedUserRef.current = null // ← reseta no logout
      }
    })
