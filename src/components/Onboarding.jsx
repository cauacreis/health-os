        ...
        <div style={{ background: 'rgba(14,14,16,0.95)', border: '1px solid rgba(220,38,38,0.12)', borderRadius: 10, padding: 28 }}>
          ...
          {step === 4 && (
            <>
              ...
              {/* ── Termos de uso ── */}
              <div style={{ padding: '14px 16px', borderRadius: 8, background: 'rgba(220,38,38,0.03)', border: `1px solid ${errors.terms ? 'rgba(239,68,68,0.4)' : 'rgba(220,38,38,0.1)'}`, transition: 'border-color 0.2s' }}>
                <label style={{ display: 'flex', gap: 12, alignItems: 'flex-start', cursor: 'pointer' }}>
                  <input
                    type="checkbox"
                    checked={termsAccepted}
                    onChange={e => { setTermsAccepted(e.target.checked); setErrors(ev => ({ ...ev, terms: undefined })) }}
                  ...
                  <span style={{ color: '#555', fontSize: 11, lineHeight: 1.7, fontFamily: "'Space Mono', monospace" }}>
                    Entendo que o Health OS é uma ferramenta de apoio à organização de treinos e nutrição.
                    As sugestões geradas têm caráter <strong style={{ color: '#444' }}>informativo</strong> e
                    não substituem a orientação de um profissional de Educação Física (CREF) ou nutricionista.
                  </span>
                </label>
                {errors.terms && (
                  <div style={{ color: '#ef4444', fontSize: 10, marginTop: 8, paddingLeft: 28 }}>
                    ⚠ {errors.terms}
                  </div>
                )}
              </div>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
          ...
          {step < STEPS ? (
            ...
            <button
              ...
              style={{
                flex: 2,
                background: termsAccepted ? 'rgba(220,38,38,0.2)' : 'rgba(255,255,255,0.02)',
                borderColor: termsAccepted ? R : '#222',
                color: termsAccepted ? R : '#333',
                boxShadow: termsAccepted ? '0 0 20px rgba(220,38,38,0.2)' : 'none',
                transition: 'all 0.3s',
              }}
            >
