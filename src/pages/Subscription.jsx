...
export default function Subscription() {
  ...
  async function fetchSubscription() {
    ...
    const { data } = await supabase.from("subscriptions").select("*").eq("user_id", session.user.id).single();
    setSub(data);
    setLoading(false);
  }
  ...
  async function handleValidateCoupon() {
    ...
    if (error || !data?.[0]) {
      ...
      setCouponMsg(r.valid
        ? { valid: true, message: `Cupom válido! ${r.discount_type === "percent" ? `${r.discount_value}% de desconto` : `R$ ${r.discount_value.toFixed(2)} de desconto`}`, discount: r }
        : { valid: false, message: r.message }
      );
    }
  ...
  async function handleCheckout() {
    ...
    try {
      const res = await fetch(`${SUPABASE_URL}/functions/v1/create-checkout`, {
        ...
        body: JSON.stringify({ coupon_code: couponMsg?.valid ? coupon.trim().toUpperCase() : undefined }),
      });
    ...
    <div style={S.page}>
      ...
      {showCancelModal && (
        <CancelModal
          ...
          onConfirm={() => { setShowCancelModal(false); setCancelStep(1); handlePortal(); }}
      ...
      <div style={S.container}>
        ...
        {/* ── Status card (usuário PRO) ─────────────────────── */}
        {!loading && (isPro || isTrialing || isPastDue) && (
          <div style={S.statusCard}>
            <div style={S.statusRow}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                {isPro && <div style={S.statusDot} />}
                <span style={S.statusBadge}>
                  {isTrialing ? "⏳ TRIAL ATIVO" : isPastDue ? "⚠️ PAGAMENTO PENDENTE" : "✦ PRO ATIVO"}
                </span>
              </div>
              {periodEnd && (
                <span style={S.statusDate}>
                  {willCancel ? `Acesso até ${periodEnd}` : `Renova em ${periodEnd}`}
                </span>
              )}
            </div>

            {willCancel && (
              <p style={S.cancelNotice}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 14, flexWrap: "wrap" }}>
              ...
              <button onClick={handlePortal} disabled={portalLoading} style={S.portalBtn}>
                ...
                <div style={S.couponSection}>
                  <div style={S.couponRow}>
                    <input
                      ...
                      maxLength={20}
                    />
                    <button onClick={handleValidateCoupon} disabled={validatingCoupon || !coupon.trim()} style={S.couponBtn}>
                      {validatingCoupon ? "..." : "OK"}
                    </button>
                  </div>
                  {couponMsg && (
                    <div style={{ ...S.couponMsg, color: couponMsg.valid ? "#4ade80" : "#f87171" }}>
                      {couponMsg.message}
                    </div>
                  )}
                </div>
                <button onClick={handleCheckout} disabled={checkoutLoading || loading} style={S.checkoutBtn}>
                  {checkoutLoading ? "Aguarde..." : "ASSINAR AGORA →"}
                </button>
                <p style={S.secureNote}>🔒 Pagamento seguro via Stripe</p>
              </>
            )}

            {isPro && <div style={{ ...S.currentBadge, color: "#dc2626", borderColor: "rgba(220,38,38,0.3)" }}>✦ PLANO ATUAL</div>}
          </div>
        </div>
        ...
        <div style={S.faq}>
          ...
          {[
            ["Posso cancelar a qualquer momento?", "Sim. Você pode cancelar a qualquer momento. O acesso Pro continua até o fim do período pago."],
...
// ── Modal de cancelamento ─────────────────────────────────────
function CancelModal({ step, setStep, periodEnd, onClose, onConfirm, portalLoading }) {
  return (
        ...
        {step === 1 && (
          <>
            <div style={M.header}>
              ...
              <p style={M.subtitle}>
                Ao cancelar, você perde acesso imediato a esses recursos no fim do período.
              </p>
            </div>
            ...
            <div style={M.footer}>
              <button onClick={onClose} style={M.keepBtn}>
                Manter meu PRO ✦
              </button>
              <button onClick={() => setStep(2)} style={M.continueBtn}>
        ...
        {step === 2 && (
          <>
            <div style={M.header}>
              ...
              <p style={M.subtitle}>
                Você será redirecionado para o portal do Stripe para confirmar o cancelamento.
                {periodEnd && ` Seu acesso PRO continua até ${periodEnd}.`}
              </p>
            </div>
            ...
            <div style={M.confirmBox}>
              <div style={M.confirmRow}>
                ...
                <span style={M.confirmText}>Acesso mantido até o fim do período</span>
              </div>
              <div style={M.confirmRow}>
              ...
              <div style={M.confirmRow}>
                <span style={{ color: "#f87171", fontSize: 12 }}>✕</span>
                <span style={M.confirmText}>Todos os recursos Pro bloqueados após {periodEnd || "o vencimento"}</span>
              </div>
            </div>

            <div style={M.footer}>
              <button onClick={() => setStep(1)} style={M.continueBtn}>
                ← Voltar
              </button>
              <button onClick={onConfirm} disabled={portalLoading} style={M.confirmBtn}>
                {portalLoading ? "Abrindo portal..." : "Confirmar cancelamento →"}
              </button>
            </div>
          </>
        )}
...
const S = {
  ...
  cancelNotice:{ fontSize: 11, color: "#f87171", margin: "8px 0 0" },
  ...
  portalBtn:   { background: "transparent", border: "1px solid #444", color: "#ccc", fontFamily: "'Space Mono', monospace", fontSize: 10, letterSpacing: 1, padding: "8px 14px", cursor: "pointer", borderRadius: 2 },
  ...
  couponInput: { flex: 1, background: "#1a1a1a", border: "1px solid #333", color: "#e5e5e5", fontFamily: "'Space Mono', monospace", fontSize: 11, letterSpacing: 2, padding: "10px 12px", borderRadius: 2, outline: "none" },
  couponBtn:   { background: "transparent", border: "1px solid #555", color: "#e5e5e5", fontFamily: "'Space Mono', monospace", fontSize: 11, padding: "0 14px", cursor: "pointer", borderRadius: 2, letterSpacing: 1 },
  couponMsg:   { fontSize: 11, marginTop: 6, letterSpacing: 0.5 },
  ...
  secureNote:  { textAlign: "center", fontSize: 10, color: "#555", margin: "8px 0 0", letterSpacing: 1 },
